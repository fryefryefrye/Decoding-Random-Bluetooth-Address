...
static void show_bonded_devices(void)
{
	...
	for (int i = 0; i < dev_num; i++) 
	{
		...
		{
			printf("%02X",*(((uint8_t*)(dev_list[i].bd_addr))+j));
		}
		printf("\r\n");
		...
		{
			printf(",0x%02X",*(((uint8_t*)(dev_list[i].bond_key.pid_key.irk))+j));
		}
		printf("\r\n");
		...
		for (unsigned char j = 0; j < sizeof(esp_bt_octet16_t); j++)
		{
			printf("%02X", *(((uint8_t*)(dev_list[i].bond_key.pid_key.irk)) + sizeof(esp_bt_octet16_t) - j -1));
		}
		printf("\r\n");
